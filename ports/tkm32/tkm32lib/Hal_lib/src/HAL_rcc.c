...
{	
	...
	RCC_PLLCmd(ENABLE);
	RCC_SYSCLKConfig(RCC_SYSCLKSource_PLLCLK);//ѡ���ⲿʱ����Ϊϵͳʱ��
}
/**
...
{
  uint32_t pllm = 0U, pllp = 0U,plln = 0U;
  ...
  switch (RCC->CFGR & RCC_CFGR_SWS)
  {
    ...
    case RCC_CFGR_SWS_PLL:  /* PLL used as system clock  source */
    {
      pllm = RCC->PLLCFGR & RCC_PLLCFGR_PLLM;
			pllp = ((RCC->PLLCFGR & RCC_PLLCFGR_PLLP) >> 4U) & 0x3U;
			if(pllp == 0x0U) 			pllp = 1;
			else if(pllp == 0x1U) pllp = 2;
			else if(0x10U)				pllp = 4;
			else 									pllp = 8;
			plln = ((RCC->PLLCFGR & RCC_PLLCFGR_PLLN) >> 6U) & 0x7FU;

      if(RCC_GET_PLL_OSCSOURCE() != RCC_PLLSOURCE_HSI)//HSE时钟或2分频时钟作为PLL输入时钟
      {
				if((RCC->CFGR & (0x1U<<16U)) != 0x0U)  //HSE 2分频
				{
					sysclockfreq = (HSE_Value>>1 * (plln+1))/((pllm+1) * pllp);
				}else //HSE 不分频
				{
					sysclockfreq = (HSE_Value * (plln+1))/((pllm+1) * pllp );
				}
      }
      else  //HSI时钟4分频时钟作为PLL输入时钟
      {
        /* HSI used as PLL clock source */
        sysclockfreq = ((HSI_VALUE >>2) * (plln+1))/((pllm+1) * pllp);
      }
	...
	clkinfo->SystemClock = sysclockfreq;
	//AHB
	clk_dat = ((RCC->CFGR & RCC_CFGR_HPRE) >> 4) & 0xFU;
	if((clk_dat & 0x8U) == 0)
	{
		clkinfo->HCLKClock = sysclockfreq;
	}
	else
	{
		clk_dat &= 0x7U;
		clkinfo->HCLKClock = (sysclockfreq >> (clk_dat+1));
	}
	//APB1

	clk_dat = ((RCC->CFGR & 0x1C00U) >> 10) & 0x7U;
	if(clk_dat > 0x3U){
		clkinfo->PCLK1Clock = (clkinfo->HCLKClock >> (clk_dat-3));
	}else{
		clkinfo->PCLK1Clock =clkinfo->HCLKClock; 
	}
	//APB2
	clk_dat = ((RCC->CFGR & 0xE000U) >> 13) & 0x7U;
	if(clk_dat > 0x3U){
		clkinfo->PCLK2Clock =(clkinfo->HCLKClock >> (clk_dat-3));
	}else{
		clkinfo->PCLK2Clock = clkinfo->HCLKClock; 
	}
	//USB
	clk_dat = ((RCC->CFGR & 0xE0000U) >> 17) & 0x7U;
	clkinfo->USBClock = (sysclockfreq /(clk_dat+1));
	
	clkinfo->QSPIClock = clkinfo->PCLK2Clock/(QSPI->SPBRG + 0x1U);
