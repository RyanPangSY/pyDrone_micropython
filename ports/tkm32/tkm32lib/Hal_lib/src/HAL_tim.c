...
void TIM_PWM_ConfigChannel(TIM_TypeDef* TIMx, TIM_OC_InitTypeDef* sConfig, uint32_t Channel)
{
...
void TIM_OC_ConfigChannel(TIM_TypeDef *htim, TIM_OC_InitTypeDef* sConfig, uint32_t Channel)
{
  /* Check the parameters */
  ...
  switch (Channel)
  {
    case TIM_CHANNEL_1:
    {
      ...
      TIM_OC1_SetConfig(htim, sConfig);
    }
    break;
    ...
    case TIM_CHANNEL_2:
    {
      ...
      TIM_OC2_SetConfig(htim, sConfig);
    }
    break;
    ...
    case TIM_CHANNEL_3:
    {
      ...
      TIM_OC3_SetConfig(htim, sConfig);
    }
    break;
    ...
    case TIM_CHANNEL_4:
    {
      ...
      TIM_OC4_SetConfig(htim, sConfig);
    }
    break;
...
void TIMEx_OCN_Start(TIM_TypeDef *htim, uint32_t Channel)
{
  /* Check the parameters */
  ...
  htim->CCER &= ~tmp;
  /* Set or reset the CCxNE Bit */
  htim->CCER |= (uint32_t)(TIM_CCxN_ENABLE << Channel);
  /* Enable the Main Output */
	htim->BDTR|=(TIM_BDTR_MOE);
...
void TIM_Encoder_Init(TIM_TypeDef *htim,  TIM_Encoder_InitTypeDef* sConfig)
{
  uint32_t tmpsmcr = 0U;
...
/**
  ...
  * @retval HAL status
  */
void TIM_Encoder_DeInit(TIM_TypeDef *htim)
{
	if((htim->CCER & TIM_CCER_CCxE_MASK) == 0U){
		if((htim->CCER & TIM_CCER_CCxNE_MASK) == 0U){
			htim->CR1 &= ~(TIM_CR1_CEN);
		}
	}
...
/**
  ...
  * @retval HAL status
  */
void TIM_Encoder_Start(TIM_TypeDef *htim, uint32_t Channel)
{
  ...
  switch (Channel)
  {
    case TIM_CHANNEL_1:
    {
      TIM_CCxChannelCmd(htim, TIM_CHANNEL_1, TIM_CCx_ENABLE);
      break;
    }
    case TIM_CHANNEL_2:
    {
      TIM_CCxChannelCmd(htim, TIM_CHANNEL_2, TIM_CCx_ENABLE);
      break;
    }
    default :
    {
...
void TIM_PWM_Start_IT(TIM_TypeDef *htim, uint32_t Channel)
{
  switch (Channel)
  {
    case TIM_CHANNEL_1:
...
void TIM_IC_Start_IT(TIM_TypeDef *htim, uint32_t Channel)
{
  switch (Channel)
  {
    case TIM_CHANNEL_1:
