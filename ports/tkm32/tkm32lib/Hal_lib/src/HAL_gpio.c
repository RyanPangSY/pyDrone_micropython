...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct)
{
  ...
  if(GPIO_InitStruct->GPIO_Pin>>16)  //˵����GPIOE��16~23λ	
  {
  ...
  assert_param(IS_GPIO_AF(GPIO_AF));
	...
	if(GPIO_Pin>>16)  //˵����GPIOE��16~23λ
	{
		...
		for(i=0;i<8;i++)
		{
			if(temp&0x01)
			{
				GPIOE->AFRH_EXT &= ~((uint32_t)0xF << ((uint32_t)(i<<2)));     //AF����ռ���ֽڣ�Ҫx4,��<<2
				GPIOE->AFRH_EXT |= ((uint32_t)GPIO_AF << ((uint32_t)(i<<2)));
			}
			temp = temp>>1;
		}
	}
	if(GPIO_Pin&0XFF00)  //˵����GPIOE��8~15λ
		{
			temp = GPIO_Pin>>8;
			for(i=0;i<8;i++)
			{
				if(temp&0x01)
				{
					GPIOx->AFRH &= ~((uint32_t)0xF << ((uint32_t)(i<<2)));     //AF����ռ���ֽڣ�Ҫx4,��<<2
					GPIOx->AFRH |= ((uint32_t)GPIO_AF << ((uint32_t)(i<<2)));
				}
				temp = temp>>1;
			}
		}
	if(GPIO_Pin&0XFF)  //˵����GPIOE��0~7λ
		{
			for(i=0;i<8;i++)
			{
				if(temp&0x01)
				{
					GPIOx->AFRL &= ~((uint32_t)0xF << ((uint32_t)(i<<2)));     //AF����ռ���ֽڣ�Ҫx4,��<<2
