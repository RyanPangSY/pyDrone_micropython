 ...
 * This program is m_free software; you can redistribute it and/or
...
STATIC uint32_t kmp_find(char* src,uint32_t src_len, const char* tagert)
{
	...
	int* next = (int*)m_malloc(sizeof(uint32_t) * tag_len);
...
bool reset(esp8285_obj* nic)
{
    ...
    if (eATRST(nic)) {
        ...
        while (mp_hal_ticks_ms() - start < 3000) {
            if (eAT(nic)) {
                mp_hal_delay_ms(1500); /* Waiting for stable */
                return true;
            }
            mp_hal_delay_ms(100);
        }
...
bool get_host_byname(esp8285_obj* nic, const char* host,uint32_t len,char* out_ip, uint32_t timeout_ms)
{
	...
	{
		mp_printf(&mp_plat_print, "[DAVINCI] %s | get_host_byname failed\n",__func__);
		return false;
	}
	char IP_buf[16]={0};
...
bool get_ipconfig(esp8285_obj* nic, ipconfig_obj* ipconfig)
{
	...
	if(cur == NULL)
	{
		mp_printf(&mp_plat_print, "[DAVINCI] %s | esp8266_ipconfig could'n get ip\n",__func__);
		return false;
	}
	
	const char *get_str = mp_obj_str_get_str(get_sscan(cur ,"ip:\"","\"",&res));
	ipconfig->ip = mp_obj_new_str(get_str,strlen(get_str));

	cur = strstr((char*)nic->buffer.buffer, "gateway");
	if(cur == NULL)
	{
		mp_printf(&mp_plat_print, "[DAVINCI] %s | esp8266_ipconfig could'n get gateway\n",__func__);
		return false;
	}
	const char *gateway_buf = mp_obj_str_get_str(get_sscan(cur ,"gateway:\"","\"",&res));
	ipconfig->gateway = mp_obj_new_str(gateway_buf,strlen(gateway_buf));
	cur = strstr((char*)nic->buffer.buffer, "netmask");
	if(cur == NULL)
	{
		mp_printf(&mp_plat_print, "[DAVINCI] %s | esp8266_ipconfig could'n get netmask\n",__func__);
		return false;
	}
	const char *netmask_buf = mp_obj_str_get_str(get_sscan(cur ,"netmask:\"","\"",&res));
	...
	const char *ssid = mp_obj_str_get_str(get_sscan(cur ,"+CWJAP_CUR:\"","\",",&res));
	ipconfig->ssid = mp_obj_new_str(ssid,strlen(ssid));
	
	const char *MAC = mp_obj_str_get_str(get_sscan(cur ,",\"","\"",&res));
...
{
    // printf("%s | head obl %d *dl %d tu %d ftr %d peer_closed %d coming_mux_id %d\n", __func__, out_buff_len, *data_len, timeout, first_time_recv);
    // printf("peer_closed %d coming_mux_id %d\r\n");
    ...
    if (size >= out_buff_len) {
        ...
        frame_sum -= size;
        // printf("Buffer_Size frame_sum %d size %d *data_len %d\n", frame_sum, size, *data_len);
                    ...
                    if (frame_len < 0) { // already frame_len - tmp_len before (not frame_len <= 0)
                        ...
                        if (tmp_state == 7 && tmp_buf[tmp_pos] == '\n') {
                            if (frame_len == -2) { // match +IPD EOF （\r\n）
                                ...
                                size = Buffer_Size(&nic->buffer);
                                // printf("%s | size %d out_buff_len %d\n", __func__, size, out_buff_len);
                                if (size >= out_buff_len) { // data enough
                                    // printf("%s | recv out_buff_len overflow\n", __func__);
                                    State = EXIT;
                                }
            ...
            ) // '+IPD,3,1452:'
            {
                if(!new_frame){
                    if(frame_len_sum > 0){
                        ...
                        // printf("%c", temp_buff[0]); //TODO: optimize uart overflow, if uart overflow, uncomment this will print some data
...
#if 1
bool myrecvFind(esp8285_obj* nic)
{
    if (recvString_1(nic, "SEND OK", 10000) != NULL)
    {
      return true;
    }
    return false;
