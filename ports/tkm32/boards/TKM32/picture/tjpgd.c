...
//__align(4) uint8_t jpg_buffer[JPEG_WBUF_SIZE];	
__ALIGN_BEGIN uint8_t jpg_buffer[JPEG_WBUF_SIZE];	
...
{
	f_jpeg=(FIL*)m_malloc(sizeof(FIL));
	if(f_jpeg==NULL)return 1;			 
...
//------------------------------------------------------------------------------
uint8_t jpg_decode(FATFS *fs, const char *filename,uint16_t x,uint16_t y,uint8_t fast)
{  
	uint8_t res=0;	
	uint8_t scale;	
	...
	if(res==0)
	{
    res=f_open(fs,f_jpeg,(const TCHAR*)filename,FA_READ);
		if(res==FR_OK)
		{ 
