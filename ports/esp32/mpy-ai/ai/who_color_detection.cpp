...
static void draw_color_detection_result(uint16_t *image_ptr, int image_height, int image_width, vector<color_detect_result_t> &results, uint16_t color)
{
    ...
    for (int i = 0; i < results.size(); ++i)
    {
		...
		{
			pCdetection.results++;
			pCdetection.piont[i].x0 = (results[i].box[0]<0) ? 0:results[i].box[0];
			pCdetection.piont[i].y0 = (results[i].box[1]<0) ? 0:results[i].box[1];
			pCdetection.piont[i].x1 = (results[i].box[2]<0) ? 0:results[i].box[2];
			pCdetection.piont[i].y1 = (results[i].box[3]<0) ? 0:results[i].box[3];
		}
	#endif
        ...
        if (xQueueReceive(xQueueFrameI, &frame, portMAX_DELAY))
        {
            if (register_mode)
            {
                ...
                case INCREASE_COLOR_AREA:
                    ...
                    ESP_LOGI(TAG,"increase color area \n");
                ...
                case DECREASE_COLOR_AREA:
                    ...
                    ESP_LOGI(TAG,"decrease color area \n");
                    dl::image::draw_hollow_rectangle((uint16_t *)frame->buf, (int)frame->height, (int)frame->width,
                                                     color_thresh_boxes[color_thresh_boxes_index][0],
                                                     color_thresh_boxes[color_thresh_boxes_index][1],
                                                     color_thresh_boxes[color_thresh_boxes_index][2],
                ...
                case CLOSE_REGISTER_COLOR_BOX:
                    ESP_LOGI(TAG,"close register color box\n");
                    xSemaphoreTake(xMutex, portMAX_DELAY);
                    register_mode = false;
                    xSemaphoreGive(xMutex);
                    break;
...
{
	if(process_handler){
		vTaskDelete(process_handler);
	}
	if(event_handler){
		vTaskDelete(event_handler);
	}
}
