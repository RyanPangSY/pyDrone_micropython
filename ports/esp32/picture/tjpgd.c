...
{	 
	jpeg_dev=(JDEC*)m_malloc(sizeof(JDEC));
	if(jpeg_dev==NULL)return 1;		
	jpg_buffer=(uint8_t*)m_malloc(JPEG_WBUF_SIZE);
	if(jpg_buffer==NULL)return 1;		
	return 0;
}
//释放内存
void jpeg_freeall(void)
{
	m_free(jpeg_dev);		//释放jpeg_dev申请到的内存
	m_free(jpg_buffer);	//释放jpg_buffer申请到的内存
}
...
{ 
    ...
    if(buf)     		
    { 
			rb = mp_stream_posix_read(jd->device, buf, num);
			return rb; 
    }else{
			rb = mp_stream_posix_lseek(jd->device, num, MP_SEEK_CUR);
			if(rb == MP_STREAM_ERROR) return 0;
			return num;
		} 
}  
...
{ 
   	...
   	for(i=0;i<height;i++)
	{
		...
		for(j=0;j<width;j++)
		{
			...
			pic_phy.draw_point(realx+picinfo.S_XOFF,realy+picinfo.S_YOFF,*pencolor);//disp pic			
			pencolor++;
		}
	}
    return 0;    
...
//------------------------------------------------------------------------------
uint8_t jpg_decode(void *fs, const char *filename,uint16_t x,uint16_t y,uint8_t fast)
{  
	uint8_t res=0;	
	uint8_t scale;	
    ...
    mp_obj_t args[2] = {
		...
		f_jpeg = mp_vfs_open(MP_ARRAY_SIZE(args), &args[0], (mp_map_t *)&mp_const_empty_map);
		
		//if(f_jpeg != NULL)
		{ 
			res=jd_prepare(jpeg_dev,jpeg_in_func,jpg_buffer,JPEG_WBUF_SIZE,f_jpeg);
			picinfo.S_Height=jpeg_dev->height;
			picinfo.S_Width=jpeg_dev->width;
