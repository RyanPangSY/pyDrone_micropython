...
#define BMP_DBUF_SIZE		2048	//最少应为LCD宽度*3
...
extern uint8_t stdbmp_decode(void *fs ,const char *filename,uint16_t x,uint16_t y);
extern uint8_t minibmp_decode(void *fs ,const char *filename,uint16_t x,uint16_t y,uint16_t width,uint16_t height,uint16_t acolor);//尺寸小于240*320的bmp图片解码.
