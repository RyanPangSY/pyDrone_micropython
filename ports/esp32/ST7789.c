  ...
  if(args[0].u_obj !=MP_OBJ_NULL) 
  {
    ...
    if (mp_obj_is_int(args[0].u_obj)) {
      mp_raise_ValueError(MP_ERROR_TEXT("lcd text parameter error"));
		...
		char * displayStr = (char * )m_malloc(bufinfo.len+1);
		memset(displayStr,0,bufinfo.len+1);

		if((text_size>>1) * bufinfo.len > lcddev.width){
			bufinfo.len = lcddev.width/(text_size>>1);
		}
			 ...
			 //---------------------------------------------------------------
				if(args[3].u_bool == true){
					...
					res = check_sys_file((const char *)file_buf);
					 if(res){
							grap_drawCached(&st7789_glcd,NULL, args[0].u_int, args[1].u_int, (const char *)file_buf); 
					 }
