...
bool fmt2jpg_t(uint8_t *src, size_t src_len, uint16_t width, uint16_t height, pixformat_t format, uint8_t quality, size_t * out_len,uint8_t * jpg_buf, uint32_t jpg_buf_len)
{
    memory_stream dst_stream(jpg_buf, jpg_buf_len);
...
bool rgb565_2jpg(camera_fb_t * fb, uint8_t quality, uint32_t jpg_buf_len, uint8_t * out, size_t * out_len)
{
    return fmt2jpg_t(fb->buf, fb->width * fb->height * 2, fb->width, fb->height, fb->format, quality, out_len, out, jpg_buf_len);
}
