...
static esp_err_t cam_dma_config()
{
	...
	cam_obj->frames = (cam_frame_t *)m_malloc( cam_obj->frame_cnt * sizeof(cam_frame_t));
...
//printf("cam obj frme:%d\r\n", cam_obj->frame_cnt);
    for (int x = 0; x < cam_obj->frame_cnt; x++) {
        ...
        cam_obj->frames[x].fb.buf = (uint8_t *)m_malloc(fb_size * sizeof(uint8_t) + dma_align);
    ...
    if (!cam_obj->psram_mode) {
		...
		cam_obj->dma_buffer = (uint8_t *)m_malloc(cam_obj->dma_buffer_size * sizeof(uint8_t));
...
esp_err_t cam_init(const camera_config_t *config)
{
	...
	//cam_obj = (cam_obj_t *)m_malloc(sizeof(cam_obj_t));
...
esp_err_t cam_deinit(void)
{
    ...
    if (cam_obj->dma_buffer) {
        m_free(cam_obj->dma_buffer);
    }
    if (cam_obj->frames) {
        for (int x = 0; x < cam_obj->frame_cnt; x++) {
            m_free(cam_obj->frames[x].fb.buf - cam_obj->frames[x].fb_offset);
            if (cam_obj->frames[x].dma) {
                free(cam_obj->frames[x].dma);
            }
        }
        m_free(cam_obj->frames);
    }
