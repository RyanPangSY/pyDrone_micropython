...
static void shutdown_everything(void)
{
   if (console.filename)
   {
      nes_free(console.filename);
      console.filename = NULL;
   }
   if (console.nextfilename)
   {
      nes_free(console.nextfilename);
...
log_printf("internal_insert file type:%d\r\n",type);

   console.filename = nes_strdup(filename);
   console.type = type;
...
{
   console.nextfilename = nes_strdup(filename);
