...
static void *mem_guardalloc(int alloc_size, int guard_size)
{
   ...
   orig = mem_alloc(alloc_size + (guard_size * 2),1);
   if (NULL == orig)
      return NULL;
...
static void mem_init(void)
{
	...
	mem_record = mem_alloc(MAX_BLOCKS * sizeof(memblock_t),0); //0
...
void *_my_malloc(int size, char *file, int line)
{
 ...
 printf("_my_malloc: %d addr:0x%x at %s:%d\r\n", size, (uint32_t)&temp,file, line);
