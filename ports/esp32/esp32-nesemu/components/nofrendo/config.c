...
static char *my_getline(FILE *stream)
{
   ...
   do
   {
      ...
      if (NULL == dynamic)
      {
         dynamic = nes_malloc(strlen(buf) + 1);
         if (NULL == dynamic)
         {
            return 0;
         }
      ...
      else
      {
         ...
         char *temp = NULL;
         temp = nes_malloc(strlen(dynamic) + strlen(buf) + 1);
         if (NULL == temp)
            return 0;
...
static int load_config(char *filename)
{
   // FILE *config_file;
	...
	if( check_sys_file((const char *)filename) )
   // if ((config_file = fopen(filename, "r")))
   {
	   ...
	   mp_obj_t config_file = mp_vfs_open(MP_ARRAY_SIZE(args), &args[0], (mp_map_t *)&mp_const_empty_map);
	   
	   printf("load_config open ok!\r\n");
      char *line;
      ...
      while ((line = my_getline(config_file)))
      {
         ...
         do 
         {
            ...
            case '[':
               ...
               if ((value = nes_malloc(strlen(group) + 1)))
               {
                  strcpy(value, group);
               }
               group = value;
      ...
      fclose(config_file);
	   ...
	   log_printf("load_config no file-->%s\r\n",filename);
   }
