...
//      bmp_destroy(&(*machine)->vidbuf);
      if ((*machine)->cpu)
      {
         if ((*machine)->cpu->mem_page[0])
            nes_free((*machine)->cpu->mem_page[0]);
         nes_free((*machine)->cpu);
      }
...
nes_t *nes_create(void)
{
   ...
   machine = nes_malloc(sizeof(nes_t));
   if (NULL == machine)
      return NULL;
...
//      goto _fail;
   ...
   /* cpu */
   machine->cpu = nes_malloc(sizeof(nes6502_context));
   if (NULL == machine->cpu)
      goto _fail;
   ...
   /* allocate 2kB RAM */
   machine->cpu->mem_page[0] = nes_malloc(NES_RAMSIZE);
