                              ...
                              int height, int pitch, int overdraw)
{
   ...
   bitmap = nes_malloc(sizeof(bitmap_t) + (sizeof(uint8 *) * height));
   ...
   if (NULL == bitmap)
	...
	//_make_bitmap height:224,width:256

	// bitmap->data = (uint8_t *)&user_buf[0];
	bitmap->data = m_malloc(height * width * (sizeof(uint8 *)));
...
bitmap_t *bmp_create(int width, int height, int overdraw)
{
   ...
   // addr = nes_malloc((pitch * height) + 3); /* add max 32-bit aligned adjustment */
   // if (NULL == addr)
      // return NULL;
